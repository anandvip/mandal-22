...
class MeditationStore {
    ...
    saveToLocalStorage() {
        try {
            ...
            handleError(error, 'Failed to save data. Please check your browser settings.', 'saveToLocalStorage');
        }
    }
    ...
    loadFromLocalStorage() {
        try {
            ...
            handleError(error, 'Failed to load saved data. Starting with a fresh state.', 'loadFromLocalStorage');
        }
    }
...
function handleCreateMandal() {
    ...
    try {
        ...
        handleError(error, 'Failed to create Mandal. Please check your inputs and try again.');
    }
}
...
function handleAddPastSession(event) {
    ...
    try {
        ...
        handleError(error, 'Failed to add past session(s)');
    }
}
...
function handleSaveNotes(event) {
    ...
    try {
        ...
        handleError(error, 'Failed to save notes');
    }
}
...
function handleAddTodaySession() {
    ...
    try {
        ...
        handleError(error, 'Failed to add today\'s session');
    }
}
...
function handleStopSession() {
    try {
        ...
        handleError(error, 'Failed to stop session');
    }
}

function enhanceMediaEmbedding() {
    ...
    try {
            ...
            if (videoId) {
                ...
                mediaContainer.innerHTML = `
                    <div class="youtube-preview">
                        ...
                        <button onclick="playYouTubeVideo('${embedUrl}', ${startTime})" class="play-button">
        ...
        handleError(error, 'Failed to embed media');
    }
}
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            try {
                ...
                handleError(error, 'Failed to import data. Please check the file and try again.');
            }
        };
...
function updateMeditationVisualization(sessions) {
    ...
    if (sessions.length === 0) {
        ...
        ctx.fillText('No meditation sessions recorded yet', width / 2, height / 2);
        return;
    }

    const sortedSessions = [...sessions].sort((a, b) => a.date - b.date);
    const startDate = sortedSessions[0].date;
    const endDate = new Date(Math.max(...sortedSessions.map(s => s.date)));
    ...
    ctx.font = '12px Arial';
    ctx.fillText('Meditation day', legendX + 60, legendY + 15);
    ctx.fillText('No meditation', legendX + 150, legendY + 15);
    ...
    canvas.addEventListener('mousemove', (event) => {
        ...
        const dayIndex = Math.floor((mouseX - startX) / cellSize);
        if (dayIndex >= 0 && dayIndex < totalDays) {
            ...
            tooltip.textContent = `${formatDate(date)}: ${sessionsOnDay.length} session(s)`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('embedMedia').addEventListener('click', enhanceMediaEmbedding);
    ...
    displaySessionHistory(store.state);
    updatePlaylistUI(store.state.playlists);
    document.body.classList.toggle('dark-mode', store.state.theme === 'dark');
    updateMeditationVisualization(store.state.sessions);
    ...
    store.addObserver({
        update: function(state) {
            ...
            updateMeditationVisualization(state.sessions);
        }
    });
